...
def cal_lap_loss(tensor_list, weight_list):
    ...
    for i in range(len(tensor_list)):
        ...
        #in_tensor = in_tensor.view(-1, 1, 3)
        in_tensor = in_tensor.reshape(-1, 1, 3)
        # 打印输入张量和卷积核的卷积核大小
        # print("Input tensor kernel size:", in_tensor.size()[-1])  # 获取输入张量的最后一个维度的大小
        # print("Kernel size:", lap_kernel.size()[-1])  # 获取卷积核的最后一个维度的大小
